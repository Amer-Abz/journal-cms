...
interface Tag extends TagFormState {
  id: number;
}
...
export default function EditTagPage({ params }: { params: { id: string }}) {
  const t = useTranslations('EditTagPage');
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(apiError);
      console.error("Error updating tag:", apiError);
    } finally {
      setIsLoading(false);
    }
  };

  if (isFetching) return <p className="p-4">{t('loadingTag')}</p>;
  if (fetchError) return <p className="p-4 text-red-500">{fetchError}</p>;

  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{t('title', { tagName: formState.name })}</h1>
        <Link href="/admin/tags" className="text-blue-500 hover:underline">
          {t('backLink')}
        </Link>
      </div>
